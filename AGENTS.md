...
- Do not use `using-superpowers`, `brainstorming`, or any other design-gate process skill.
...
Do not propose alternative implementations (Python CLI, manual steps, extended scripts, etc.).
The shell script is the canonical, complete implementation of the setup workflow.
...
1. Run `git branch --show-current`
   ...
   - Do not proceed until on a `patch/*` branch
...
3. Run `git fetch upstream && git log main..upstream/main --oneline`
   ...
   - Do not start new work until the user has confirmed whether to sync first
...
```
Add a row to `PATCHES.md` immediately after creating the branch.
